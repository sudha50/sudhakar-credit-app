...
void shouldGetMerchantById_returns404_notFound() throws Exception {
    ...
    when(merchantService.getMerchantById(nonExistentId)).thenThrow(new ResourceNotFoundException("Not found"));
...
void shouldGetAllMerchants_returns200() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/merchants"))
        ...
        .andExpect(jsonPath("$[0].category").value("Retail"));
}
...
void shouldGetMerchantsByCategory_returns200() throws Exception {
    ...
    MerchantDTO merchant = new MerchantDTO();
    merchant.setName("Best Electronics");
    merchant.setCategory(category);
    List<MerchantDTO> merchants = Collections.singletonList(merchant);
    when(merchantService.getMerchantsByCategory(category)).thenReturn(merchants);

    // When
    mockMvc.perform(get("/api/v1/merchants/category/{category}", category))
        // Then
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].name").value("Best Electronics"))
...
void shouldCreateMerchant_returns400_invalidInput() throws Exception {
    // Given
    MerchantDTO invalidMerchant = new MerchantDTO();
    invalidMerchant.setName(""); // Invalid: name is @NotBlank
    invalidMerchant.setCategory(""); // Invalid: category is @NotBlank

    // When
    mockMvc.perform(post("/api/v1/merchants")
            .contentType("application/json")
            .content(new ObjectMapper().writeValueAsString(invalidMerchant)))
...
void shouldUpdateMerchant_returns200() throws Exception {
    ...
    mockMvc.perform(put("/api/v1/merchants/{id}", 1L)
            .contentType("application/json")
            .content("{\"name\":\"Updated Merchant\", \"category\":\"Retail\"}"))
            .andExpect(status().isOk())
